...
TRAINING_HORIZON = '150D'
FORECAST_HORIZON = '1D' #'1D'   '1H'
FORECAST_HORIZON_LARGE = '7D' #'1D'   '1H'
TRAINING_FREQUENCY = '7D'
REQUESTED_INTERVAL_R2 = '45D'

ATTRIBUTE_TO_FORECAST = 'Intensity'
MODEL_FILE_NAME = 'finalized_model'
CSV_FILE_NAME = ""
TYPES_OF_SENSOR = ["thermal", "elec", "bus", "moto"]

# TYPES_OF_SENSOR = [  "HOPac67b2ce5312_noise" ,"HOPac67b2cd1cfe_noise", "HOPac67b2d5c9ee_noise"]

# URL definition

URL_FORECAST  = "https://tip-imredd.unice.fr/data/imredd/nice/traffic/entityOperations/upsert?api-key=978c0962-c4e3-46f7-867d-9932d05d7987"

TIME_TO_WAIT_FOR_TABLE_UPDATE = 60*60 #in seconds
...
if (TYPE_REQUEST==1):
    URL_PART1 = 'https://tip-imredd.unice.fr/data/imredd/nice/traffic/temporal/entities/?api-key=978c0962-c4e3-46f7-867d-9932d05d7987&type=https://smart-data-models.org/dataModel.Transportation/TrafficFlowObserved&id=https://api.nicecotedazur.org/nca/mobility/traffic/Promenade/manuel/'
    URL_PART2 = '&timeproperty=modifiedAt&options=sysAttrs&lastN=20&timerel=after&timeAt='
else:        
    URL_PART1 = 'https://tip-imredd.unice.fr/data/imredd/nice/traffic/temporal/entities/?api-key=978c0962-c4e3-46f7-867d-9932d05d7987&type=https://smart-data-models.org/dataModel.Transportation/TrafficFlowObserved&id=https://api.nicecotedazur.org/nca/mobility/traffic/Promenade/manuel/'
    URL_PART2 = '&timeproperty=modifiedAt&options=sysAttrs&lastN=20&timerel=before&timeAt='

FORECAST_ID_PART1 = "https://api.nicecotedazur.org/forecast/nca/mobility/traffic/Promenade/forecast/"
FORECAST_ID_PART2 = ""
...
def get_and_process_new_data_with_forecast_recurrent(time_to_wait_for_update,csv_file_name):
    ...
    #     time_to_wait_for_update = 60
    # else :
    #     time_to_wait_for_update = TIME_TO_WAIT_FOR_TABLE_UPDATE
    #     response = x.content
    #     jsondata = json.loads(response)
    #     array = []
    #     with open(csv_file_name+".csv", "r") as scraped:
    #         final_time = scraped.readlines()[-1].split(',')[0]
    #     final_datetime = datetime.strptime(final_time, '%m/%d/%Y %H:%M')  
    #     csvfile = open(csv_file_name+".csv", 'a', newline='')
    #     writer = csv.writer(csvfile) #, quoting=csv.QUOTE_NONE, delimiter=',', quotechar='')
    #     nb_rows_added = 0
    #     for i in reversed(range(len(jsondata[0]['Intensity']))):
    #             booldate_ok=1
    #             try:
    #                     datetime.strptime(jsondata[0]['Intensity'][i]['modifiedAt'], '%Y-%m-%dT%H:%M:%S.%fZ')
    #             except ValueError:
    #                 try:
    #                     datetime.strptime(jsondata[0]['Intensity'][i]['modifiedAt'], '%Y-%m-%dT%H:%M:%SZ')
    #                     booldate_ok=2
    #                 except ValueError:
    #                     booldate_ok=0
    #             if booldate_ok==1:
    #                     datetime_object = datetime.strptime(jsondata[0]['Intensity'][i]['modifiedAt'], '%Y-%m-%dT%H:%M:%S.%fZ') + timedelta(hours=2)
    #             elif booldate_ok==2:
    #                     datetime_object = datetime.strptime(jsondata[0]['Intensity'][i]['modifiedAt'], '%Y-%m-%dT%H:%M:%SZ') + timedelta(hours=2)
    #             if booldate_ok>0 and datetime_object>final_datetime+timedelta(minutes=1):
    #                     newtime = datetime_object.strftime('%m/%d/%Y %H:%M')
    #                     newvalue = jsondata[0]['Intensity'][i]['value']
    #                     writer.writerow([newtime,newvalue])
    #                     nb_rows_added = nb_rows_added+1
    #     csvfile.close()
    #     print("we filled: ", nb_rows_added, " rows")
        # for type_of_sensor in TYPES_OF_SENSOR:
        #     process_data(csv_file_name+".csv", AGGREGATION, PROCESSED_DATA_FILE+type_of_sensor+"_aggregation_"+ AGGREGATION +".csv" )
        # # process_data(csv_file_name, AGGREGATION, PROCESSED_DATA_FILE )
...
def get_new_data_once_using_before_request(url,csv_file_name):
    ...
    if (x.status_code <200 or x.status_code >299):
        ...
        if isinstance(jsondata[0]['Intensity'], list):
            for i in reversed(range(len(jsondata[0]['Intensity']))):
                    booldate_ok=1
                    try:
                            datetime.strptime(jsondata[0]['Intensity'][i]['modifiedAt'], '%Y-%m-%dT%H:%M:%S.%fZ')
                ...
                booldate_ok=1
                try:
                        datetime.strptime(jsondata[0]['Intensity']['modifiedAt'], '%Y-%m-%dT%H:%M:%S.%fZ')
                except ValueError:
                    try:
                        datetime.strptime(jsondata[0]['Intensity']['modifiedAt'], '%Y-%m-%dT%H:%M:%SZ')
                        booldate_ok=2
                    except ValueError:
                        booldate_ok=0
                if booldate_ok==1:
                        datetime_object = datetime.strptime(jsondata[0]['Intensity']['modifiedAt'], '%Y-%m-%dT%H:%M:%S.%fZ') + timedelta(hours=2)
                elif booldate_ok==2:
        ...
        # x = requests.get(url_full)
            ...
            if (x.status_code <200 or x.status_code >299):
                ...
                # print(jsondata[0]['LAeq'][0]['value']) 
                    ...
                    if latest_time > date_time_now - timedelta(hours=1):
                        booleen_update_incomplete = 0
                    csvfile.close()
...
def fill_gap(df, max_time_interval_second, type_of_sensor, aggregation, forecast_horizon, plot=False):
    ...
    print("interpolation for the end of data from: ", start_time)
    next_stop_datetime = next_stop.astype(datetime)
    stop_timestamp = int(stop.timestamp())


    if stop_timestamp > int(next_stop_datetime): # if we are missing at least one data between the end of the database and now
...
def send_to_context_broker(url_forecast, forecast_dataframe,typeofsensor):
    ...
    "type": "https://smart-data-models.org/dataModel.Environment/TrafficEnvironmentImpactForecast",
    "dateCreated": {
        "type": "Property",
        "value": date_forecast
    },
    ...
    "address": {
        "type": "Property",
        "value": {
            "streetAddress": "Promenade des Anglais,140",
    ...
    "traffic": {
        ...
        "value": [
            {
                "vehicleClass": typeofsensor,
                "intensityExpected": Traffic_forecast,
                "occupancyExpected": 0.76,
                "averageVehicleSpeedExpected": 45
            },
        ]
    },
...
@app.get("/process/greenmov/traffic")
...
@app.get("/updatedb/greenmov/traffic")
...
@app.get("/trainforecasting/greenmov/traffic")
...
@app.get("/forecast/greenmov/traffic")
...
@app.get("/forecast/greenmov/traffic/single")
...
@app.get("/forecast/greenmov/traffic/json")
...
@app.get("/forecast/greenmov/traffic/json/single")
